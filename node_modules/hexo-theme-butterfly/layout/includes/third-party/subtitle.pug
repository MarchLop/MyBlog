  ...
  when 1
    script.
      function subtitleType () {
        fetch('https://v1.hitokoto.cn?c=a').then(r => r.json()).then(d => { typedJSFn.processSubtitle(d.hitokoto, ['出自 '+d.from]); }).catch(() => fetch('https://v1.hitokoto.cn?c=c').then(r => r.json()).then(d => { typedJSFn.processSubtitle(d.hitokoto, ['出自 '+d.from]); }).catch(err => { console.error('Failed:', err); typedJSFn.processSubtitle(!{JSON.stringify(subContent)}); }))
